...
### 4. التحقق من حالة الدفع (التحقق من عملية واحدة)
استخدم هذا المسار بعد عودة المستخدم من بوابة الدفع للتأكد من حالة العملية (سواء نجحت أو لا تزال معلقة).
*   **المسار**: `GET /payments/:id`
*   **الوصف**: استبدل `:id` بالمعرف الذي استلمته في رابط العودة (`orderId` أو `transactionId`).

> [!TIP]
> **ملاحظة تقنية**: تحديث الحالة يتم عبر الـ (Webhook) وقد يتطلب ثانية واحدة. إذا كانت الحالة `pending` عند أول طلب، أعد المحاولة بعد ثانية.
...
```json
{
  ...
  "data": {
    "_id": "65f...",
    "status": "succeeded", // هامة للتأكد من اكتمال الدفع
    "amount": 1000,
    "provider": "paymob",
    "book": {
      "id": "65f...",
...
## 💡 ملاحظات تقنية:
1.  **الأسعار**: يتم التعامل بالوحدات الكاملة للعملة (مثل الجنيه أو الدولار). إذا كان السعر `100` يعني **100** جنيه أو دولار وليس سنتات.
...
4.  **حالات الدفع (Status)**:
    ...
    *   `requires_payment_method`: بانتظار إدخال بيانات الدفع (Stripe).
